...
# استيراد ملفات الإصلاح الإضافية
try:
    from fix_admin_access import fix_admin_access
    from fix_auth_routes import fix_auth_routes
except ImportError as e:
    logger = logging.getLogger(__name__)
    logger.warning(f"لم يتم العثور على بعض ملفات الإصلاح الإضافية: {str(e)}")
    
    # تعريف دوال فارغة للإصلاحات غير الموجودة
...
# تطبيق إصلاح وصول المسؤول
try:
    fix_admin_access()
    logger.info("تم تطبيق إصلاح وصول المسؤول")
except Exception as e:
    logger.error(f"فشل تطبيق إصلاح وصول المسؤول: {str(e)}")

# تطبيق إصلاح مسارات المصادقة
try:
    fix_auth_routes()
    logger.info("تم تطبيق إصلاح مسارات المصادقة")
except Exception as e:
    logger.error(f"فشل تطبيق إصلاح مسارات المصادقة: {str(e)}")

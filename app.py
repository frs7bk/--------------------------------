    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.check_password(password):
                ...
                return redirect(next_page or url_for('admin_dashboard_page'))
            else:
...
def admin_dashboard_page():
    try:
        ...
        services = Service.query.all()
        services_count = len(services)  # Fix: count the length of services list instead of passing the list itself
        ...
        # Fix: check for both 'home' and 'hero' sections for compatibility
        home_section = Section.query.filter_by(name='home').first()
        if not home_section:
            home_section = Section.query.filter_by(name='hero').first()
        ...
        # Add check for unread messages and new service requests
        ...
        try:
            ...
            new_requests_count = ServiceRequest.query.filter_by(status='new').count()
        except Exception as e:
            app.logger.error(f"Error counting messages and requests: {str(e)}")
        ...
        return render_template('admin/error.html', 
                              error_code=500, 
                              error_message="حدث خطأ أثناء تحميل لوحة التحكم",
                              error_details=str(e))
